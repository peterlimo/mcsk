...
//print_r($dbResults[0][3]);exit();
        ...
        if (isset($dbResults[0][3]) && $dbResults[0][3] != 'null') {
            //data exists
            echo $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" . "A conflict file found in local database <br />";
...
//print_r($ThisFileInfo);exit();
                $uploadId = saveUpload($ThisFileInfo['comments'], null, $file,$i);
                if (isset($uploadId)) {

                    foreach ($dbResults as $conflict) {
                        $uploaded = saveResults($ThisFileInfo['comments'], $conflict, $uploadId,$i);
                    }
                } else {
                    $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" . "An error occured when saving to results <br />";
                }
            } else {
                echo $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" . "An error occured when saving metadata <br />";
            }
...
// Initialize getID3 engine
                ...
                $id = getFileIdentifier($filename);
                if ($id == null) {
                    $_SESSION['failure'] .="<strong>" . $filename . "</strong>" .  "An error occured when getting fingerprint id <br />";
                } else {
                    $stored = file_put_contents($metaDir . $id . '.json', json_encode($ThisFileInfo['comments']));
                    if ($stored) {
                        //save to db
                        $uploadId = saveUpload($ThisFileInfo['comments'], $result['1'], $file,$i);

                        if (isset($uploadId)) {

                            $_SESSION['success'] .= "<strong>" . $filename . "</strong>" ." Processed successfully <br />";
                        } else {
                            $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" ." An error occured when saving to uploads <br />";
                        }
                    } else {
                        echo $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" . " An error occured when saving metadata <br />";
//exit();
                ...
                $_SESSION['failure'] .="<strong>" . $filename . "</strong>" . " An error occured when saving fingerprint <br />";
            }
        } else {
            $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" ." An error occured when querying the database for matches <br />";
        }
...
//print_r($result);exit();
    ...
    if (empty($result)) {
        $_SESSION['failure'] .= "<strong>" . $file . "</strong>" . " Failed to query db <br />";
...
function saveUpload($data, $duration, $fileName,$i) {
//Array ( [album] => Array ( [0] => TAMU TAMU ) [artist] => Array ( [0] => Alex Waweru ) [band] => Array ( [0] => Alex Waweru; ) [title] => Array ( [0] => Tamu tamu ) [track_number] => Array ( [0] => 1 ) [year] => Array ( [0] => 1998 ) )
 ...
 $data_to_store['album'] = $_POST['album'][$i];
}
    ...
    $data_to_store['stage_name'] = $_POST['stageName'][$i];
    $data_to_store['membership_number'] = $_POST['memberNo'][$i];
...
function saveResults($datapassed, $conflict, $uploadId,$i) {
//Array ( [album] => Array ( [0] => TAMU TAMU ) [artist] => Array ( [0] => Alex Waweru ) [band] => Array ( [0] => Alex Waweru; ) [title] => Array ( [0] => Tamu tamu ) [track_number] => Array ( [0] => 1 ) [year] => Array ( [0] => 1998 ) )
    $data_to_store = array();
...
//print_r($data_to_store);exit();
    if (isset($last_id)) {
        ...
        $_SESSION['failure'] .= "<strong>" . $filename . "</strong>" . "An error occured when saving results <br />";
        return null;
    }
}
